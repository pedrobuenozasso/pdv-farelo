 ...
 * negócio, só consultar + logar), a cobertura de valor está em garantir que
 * a resposta é interpretada corretamente e que entradas malformadas falham
 * de forma clara — não em orquestrar Testcontainers/backend real a partir
 * de `apps/edge-agent`. A prova de ponta a ponta (cliente HTTP + parsing
 * contra a API de verdade) é feita manualmente (ver README.md /
 * relato do ticket), como qualquer outro esqueleto deste tamanho no
 * repositório.
 *
...
import { parsePrintJobs, formatPrintJobLog } from "./printJobsClient.js";
